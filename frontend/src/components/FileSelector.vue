<template>
    <div>
        <input ref="input" type="file" :id="inputId" accept="*/*" @change="handleFiles">
        <label :for="inputId" class="file-content" v-if="currentFile">{{currentFile.name}}</label>
        <label v-else class="avatar-btn-add" :for="inputId"></label>
    </div>
</template>
...
const inputId = `uploader-${Math.random().toString(36).slice(2, 10)}`
