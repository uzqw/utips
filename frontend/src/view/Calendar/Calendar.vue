<template>
    <div class="statistic-container">
        <PageHeader :title="$tt('日历')"/>

        <div class="calendar-container" :style="`height:${uiStore.insets.heightPanel}px`">

            <div class="calendar" :style="`height:${uiStore.insets.heightPanel - 40}px`">
                <Calendar
            ...
            <div class="operation-panel">
                <ButtonSmall @click="getDiaries" class="mb-2">{{ $tt('日历 - 日记') }}</ButtonSmall>
                <ButtonSmall @click="loadCalendarPeriod" class="mb-2">{{ $tt('日历 - 经期') }}</ButtonSmall>
                <ButtonSmall v-if="periodDiary?.id" @click="router.push({name: 'Edit', params: {id: periodDiary.id}})" class="mb-2">{{ $tt('编辑经期日历') }}</ButtonSmall>
...
import {DiaryEntity, DiarySearchParams} from "@/types/diary.ts";
        ...
        .then(res => {
            ...
            diaries.value.forEach(item => {
                attributes.value.push({
                    ...
                    bar: {
                        style: {
                            backgroundColor: categoryStore.categoryObjectMap.get(item.category).color
                        }
                    },
                    color: categoryStore.categoryObjectMap.get(item.category).color,
...
const {isListNeedBeReload} = storeToRefs(diaryStore)
