    ...
    <div class="diary-header">
        ...
        <div class="date-wrapper" v-if="props.diary.dateObj">
            ...
            <div class="date-lunar">
                <template v-if="locale === 'zh-CN'">{{lunarObject.IMonthCn}}{{lunarObject.IDayCn}}  </template>{{ translateText(props.diary.dateObj.weekday) }}
                <span
                    v-if="Moment().diff(props.diary.date, 'days') > 0"
                    class="date-diff"
                >
                    {{Moment().diff(props.diary.date, 'days')}} {{ translateText('天前') }}
                </span>
            </div>
        </div>
        ...
        <div class="weather-wrapper">
            <div class="weather">
                <img v-if="props.diary.weather" :src="WeatherIcons[`${props.diary.weather}_active`]"
