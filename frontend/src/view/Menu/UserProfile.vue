    ...
    <div class="user-profile">
        ...
        <div class="user-info mt-1 mb-2">
            <div class="user">
                ...
                <div class="operation">
                    <div class="logout" @click="logout">{{ $tt('退出') }}</div>
                    <div class="logout ml-3" @click="changeProfile">{{ $tt('修改') }}</div>
                </div>
            </div>
        </div>
        <div v-if="categoryStore.statisticsCategory.shared > 0" class="statistics">
            <p>📍 {{userInfo.city}}</p>
            <p>{{ $tt('总计') }} <b>{{ categoryStore.statisticsCategory.amount }}</b> {{ $tt('篇') }}</p>
            <p>{{ $tt('共享') }} <b>{{ categoryStore.statisticsCategory.shared }}</b> {{ $tt('篇') }}</p>
        </div>
        <div class="copyright">
