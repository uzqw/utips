            ...
            <div class="body-login" v-if="show">
                <div class="logo-wrapper mb-6">
                    <label class="logo avatar" for="avatar">
                        <img v-if="formUser.avatar" :src="formUser.avatar + '-' + projectConfig.QiniuStyleSuffix || LogoIcons.logo_avatar" alt="Diary Logo">
                ...
                <form id="changeProfileForm">
                    <div class="input-group">
                        <label for="nickname">{{ $tt('昵称') }}</label>
                    ...
                    <div class="input-group">
                        <label for="geolocation">{{ $tt('经纬度') }}</label>
                    ...
                    <button class="btn mt-8 btn-active"
                            type="button"
                            @click.prevent="changeProfileSubmit">{{ $tt('确定修改') }}
                    </button>
                </form>
                <div class="footer flex-start">
                    <div class="link" @click="router.go(-1)">{{ $tt('返回') }}</div>
                </div>
            </div>
        </transition>
    </div>
...
import {UserProfileEntity} from "@/types/User.ts";
