    ...
    <div class="date-selector">
        <div class="date-set-item">
            ...
            >
                ...
                <template #footer>
                    <div class="p-2">
                        <ButtonSmall @click="moveToday">{{ $tt('今天') }}</ButtonSmall>
                    </div>
                </template>
            </DatePicker>
        ...
        <div class="date-meta">
            <div class="lunar" v-if="locale === 'zh-CN'">{{lunarObject.IMonthCn}}{{lunarObject.IDayCn}}</div>
            <div class="weekday">{{ translateText(lunarObject.ncWeek) }}</div>
        </div>
    </div>
...
const datePickerLocale = computed(() => locale.value === 'zh-CN' ? 'zh' : 'en')
...
const popoverOptions = ref({
    visibility: 'click' as const, // When the popover appears
    placement: 'bottom' as const, // Where the popover appears
