<template>
    <div class="statistic-container" :style="`height: ${uiStore.insets.windowsHeight}px`">

        <!-- Header -->
        <PageHeader :title="$tt('统计数据')">
            <div class="main-statistic">
                <div class="main-statistic-item">
                    <div class="label">{{ $tt('共享') }}</div> <div class="number value">{{ categoryStore.statisticsCategory.shared}}</div>
                </div>
                <div class="main-statistic-item">
                    <div class="label">{{ $tt('总计') }}</div> <div class="number value">{{ categoryStore.statisticsCategory.amount }}</div>
                </div>
            </div>
        </PageHeader>
        ...
        <div v-else class="statistic-content" :style="`height:${uiStore.insets.heightPanel}px`">
...
function getStatistic() {
    statisticApi
        ...
        .then(res => {
            ...
            categoryStore.dataArrayCategory =  keys.map(key => {
                return {
                    name: categoryStore.categoryNameByEnMap.get(key),
                    key: key,
                    value: res.data[key]
                }
            })
    ...
    statisticApi
        ...
        .then(res => {
            ...
            if (res.data){
                categoryStore.dataArrayYear = res.data.reverse().map(year => {
                    return {
                        name: year.year,
                        value: year.count
                    }
