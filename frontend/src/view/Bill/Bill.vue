<template>
    <div class="statistic-container">
        <PageHeader :title="$tt('账单')"/>
        <div class="bill-content" :style="`height:${uiStore.insets.heightPanel}px`">
            <div class="bill-container" >
                <div class="filter-panel">

                    <!--  筛选面板 -->
                    <BillPanel :width="350" :title="$tt('账单筛选')" padding="10px 15px">
                        <div class="input-group white">
                            <label for="bill-keyword" >{{ $tt('关键字') }}</label>
                            <textarea rows="3"
                                      :placeholder="$tt('支持多个关键字，以空格间隔')"
                        ...
                        <Button class="mb-2" @click="getBillData">{{ $tt('搜索') }}</Button>
                        <ButtonSmall class="mb-2" @click="getBillKeys">{{ $tt('获取最新账单类目') }}</ButtonSmall>
                        <ButtonSmall class="mb-2"  @click="hideBigIncome" v-if="isShowSalaryButton">{{ $tt('结果隐藏工资收入') }}</ButtonSmall>
                    </BillPanel>
...
function hideBigIncome(){
    billYearData.value.forEach(monthData => {
        ...
        monthData.days.forEach(dayItem => {
            // 带有工资/salary的账单条目
            let salaryArray: Array<EntityBillItem> = dayItem.items.filter(item => {
                const name = item.item.toLowerCase()
                return name.includes('工资') || name.includes('salary')
            })
            // 获取工资收入条目总合：日
            let salaryAmountDay = salaryArray.reduce((sum,nextItem) => sum + nextItem.price, 0)
            // 过滤工资/salary条目
            dayItem.items = dayItem.items.filter(item => {
                const name = item.item.toLowerCase()
                return !name.includes('工资') && !name.includes('salary')
            })
            // 日收入
