            ...
            <div class="body-login" v-if="show">
                <div class="logo-wrapper">
                    <div :class="['logo', {valid: avatarLink} ]">
                        ...
                        <img v-else :src="LogoIcons.logo_rounded" alt="LOGIN-LOGO">
                    </div>
                </div>

                <form method="post" id="regForm" @submit.prevent="loginSubmit">
                    <div class="input-group">
                        <label for="email" :class="{red: emailVerified || email.length < 1}">{{ translateText(labelEmail) }}</label>
                        <input v-model.lazy="email" type="text" name="email" id="email">
                    </div>
                    <div class="input-group">
                        <label for="password">{{ $tt('密码') }}</label>
                        <input v-model="password" name="password" type="password" id="password">
                    </div>
                    <button class="btn mt-8" :class="verified ? 'btn-active' : 'btn-inactive'" type="submit">{{ translateText(loginLabel) }}</button>
                </form>
                 <div :class="['footer', {center: !isShowDemoAccount}]">
                    <RouterLink to="/register">{{ $tt('注册') }}</RouterLink>
                    <a v-if="isShowDemoAccount" @click="useTestAccount">{{ $tt('试用演示账户') }}</a>
                </div>
                <div class="copyright">
                    <!-- <span class="version ml-1">v{{ packageInfo.version }}</span> -->
                    <template v-if="projectConfig.icpBeianNumber">
                        <a class="project-name" href="https://beian.miit.gov.cn/" target="_blank">{{ projectConfig.icpBeianNumber }}</a>
                        <span class="ml-1"> | </span>
                    </template>
                    <!-- <span class="ml-1">  始于 {{ packageInfo.dateInit }}</span> -->
                    <RouterLink class="ml-1 password" to="/fetch-password">{{ $tt('找回密码') }}</RouterLink>
                </div>
            </div>
