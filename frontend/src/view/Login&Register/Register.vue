        ...
        >
            <div class="body-login" v-if="show">
                <div class="logo-wrapper">
                    <div class="logo">
                        <img :src="LogoIcons.logo_register" alt="Diary Logo">
                    </div>
                    <div class="project-name">{{ $tt('注册') }}</div>
                </div>
                <RegisterTip :html-content="projectConfig.registerTip"/>

                <form id="regForm">
                    <div class="input-group">
                        <label for="invitation" :class="{red: !invitationVerified}">{{ translateText(labelInvitation) }}</label>
                    ...
                    <div class="input-group">
                        <label for="nickname" :class="{red: !nicknameVerified}">{{ translateText(labelUsername) }}</label>
                        <input v-model="nickname" type="text"
                               name="nickname" id="nickname">
                    </div>
                    <div class="input-group">
                        <label for="email" :class="{red: !emailVerified}">{{ translateText(labelEmail) }}</label>
                        <input v-model.lazy="email"
                               type="text" name="email" id="email">
                    </div>
                    <div class="input-group">
                        <label for="password1" :class="{red: !password1Verified}">{{ translateText(labelPassword1) }}</label>
                        <input v-model.lazy="password1"
                               name="password1" type="password" id="password1">
                    </div>
                    <div class="input-group">
                        <label for="password2" :class="{red: !password2Verified}">{{ translateText(labelPassword2) }}</label>
                        <input v-model="password2"
                               type="password" name="password2" id="password2">
                    </div>
                    <button class="btn mt-8" :class="verified ? 'btn-active' : 'btn-inactive'"
                            type="button"
                            @click.prevent="regSubmit">{{ $tt('注册') }}
                    </button>
                </form>
                <div :class="['footer', {center: !projectConfig.isShowDemoAccount}]">
                    <RouterLink to="/login">{{ $tt('登录') }}</RouterLink>
                </div>
            </div>
        </transition>
    </div>
...
import {UserRegisterEntity} from "@/types/User.ts";
